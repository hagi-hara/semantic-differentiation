...
d %>% 
  ...
  summarise(ageM = first(ageM),
            ...
            noun = first(noun),
            verb = first(verb),
            PropLook_Correct = mean(PropLook_Correct, na.rm = TRUE),
            .groups = "drop") %>%
  mutate(PropLook_Correct = if_else(PropLook_Correct >= 0.999, 0.9999999999, PropLook_Correct),
...
data <- list(Nrow   = nrow(df),
             PropLook = df$PropLook_Correct,
             X1 = df$vcbAll,
             X2 = if_else(df$Condition == "ObjAbs", 1, 0),
             X3 = df$vcbAll * if_else(df$Condition == "ObjAbs", 1, 0),
             N_subject  = length(unique(df$id)),
             ID_subject = df$ID_num)

## MCMC sampling with cmdstanr
here("Study1", "Models", "Continuous", "model1.stan") %>%
...
output_dir <- here("Study1", "MCMCSamples", "Analysis2_PreferentialLooking_Continuous_VocabularySize", "model1")
...
str_c(output_dir,
  ...
  read_stan_csv() -> fit1
print(fit1, pars = c("beta0", "beta1", "beta2", "beta3", "sigma", "f_kappa"), probs = c(0.5, 0.025, 0.975))
...
## MCMC sampling with cmdstanr
here("Study1", "Models", "Continuous", "model2.stan") %>% 
...
output_dir <- here("Study1", "MCMCSamples", "Analysis2_PreferentialLooking_Continuous_VocabularySize", "model2")
if (!dir.exists(output_dir)){
  dir.create(output_dir, recursive = TRUE)
}
...
str_c(output_dir,
      list.files(output_dir, pattern = ".csv"), sep = "/") %>% 
  read_stan_csv() -> fit2
print(fit2, pars = c("beta0", "beta1", "beta2", "sigma", "f_kappa"), probs = c(0.5, 0.025, 0.975), digits=3)
...
## MCMC sampling with cmdstanr
here("Study1", "Models", "Continuous", "model3_4.stan") %>% 
...
output_dir <- here("Study1", "MCMCSamples", "Analysis2_PreferentialLooking_Continuous_VocabularySize", "model3")
if (!dir.exists(output_dir)){
  dir.create(output_dir, recursive = TRUE)
}
...
str_c(output_dir,
      list.files(output_dir, pattern = ".csv"), sep = "/") %>% 
  read_stan_csv() -> fit3
print(fit3, pars = c("beta0", "beta1", "sigma", "f_kappa"), probs = c(0.5, 0.025, 0.975))
...
output_dir <- here("Study1", "MCMCSamples", "Analysis2_PreferentialLooking_Continuous_VocabularySize", "model4")
if (!dir.exists(output_dir)){
  dir.create(output_dir, recursive = TRUE)
}
...
str_c(output_dir,
      list.files(output_dir, pattern = ".csv"), sep = "/") %>% 
  read_stan_csv() -> fit4
print(fit4, pars = c("beta0", "beta1", "sigma", "f_kappa"), probs = c(0.5, 0.025, 0.975))
...
## MCMC sampling with cmdstanr
here("Study1", "Models", "Continuous", "model5.stan") %>% 
...
output_dir <- here("Study1", "MCMCSamples", "Analysis2_PreferentialLooking_Continuous_VocabularySize", "model5")
if (!dir.exists(output_dir)){
  dir.create(output_dir, recursive = TRUE)
}
...
str_c(output_dir,
      list.files(output_dir, pattern = ".csv"), sep = "/") %>% 
  read_stan_csv() -> fit5
print(fit5, pars = c("beta0", "sigma", "f_kappa"), probs = c(0.5, 0.025, 0.975))
...
df_waic %>% 
  ...
  arrange(desc(Estimate)) # Table S5
...
data <- list(Nrow   = nrow(df),
             ...
             X2_pred = if_else(df$Condition == "ObjAbs", 1, 0))

## MCMC sampling with cmdstanr
here("Study1", "Models", "Continuous", "model5_selected.stan") %>% 
  cmdstan_model() -> model
...
output_dir <- here("Study1", "MCMCSamples", "Analysis2_PreferentialLooking_Continuous_VocabularySize", "model_selected")
if (!dir.exists(output_dir)){
  dir.create(output_dir, recursive = TRUE)
}
...
df %>% 
  ...
  geom_jitter(aes(y = PropLook_Correct, color = Condition), shape = 16, size = 2, width = 0.15, height = 0, alpha = 0.5) + 
  ...
  labs(x = "Total vocabulary size", y = "Mean proportion of looking at the correct stimulus") +
...
ggsave(file = here("Study1", "Figures", "Analysis2_PreferentialLooking_Continuous_VocabularySize.png"), plot = gp, dpi = 350, width = 5.1, height = 4.9) # Figure S3b(left)
