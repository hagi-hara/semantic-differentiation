...
output_dir <- here("Study1", "MCMCSamples", "Analysis1_PreferentialLooking_Binary_Age", "model1")
...
output_dir <- here("Study1", "MCMCSamples", "Analysis1_PreferentialLooking_Binary_Age", "model2")
if (!dir.exists(output_dir)){
  dir.create(output_dir, recursive = TRUE)
}
...
output_dir <- here("Study1", "MCMCSamples", "Analysis1_PreferentialLooking_Binary_Age", "model3")
if (!dir.exists(output_dir)){
  dir.create(output_dir, recursive = TRUE)
}
...
output_dir <- here("Study1", "MCMCSamples", "Analysis1_PreferentialLooking_Binary_Age", "model4")
if (!dir.exists(output_dir)){
  dir.create(output_dir, recursive = TRUE)
}
...
output_dir <- here("Study1", "MCMCSamples", "Analysis1_PreferentialLooking_Binary_Age", "model5")
if (!dir.exists(output_dir)){
  dir.create(output_dir, recursive = TRUE)
}
...
output_dir <- here("Study1", "MCMCSamples", "Analysis1_PreferentialLooking_Binary_Age", "model_selected")
if (!dir.exists(output_dir)){
  dir.create(output_dir, recursive = TRUE)
}
...
df %>%
  ...
  ungroup() %>% 
  filter(!is.na(Prop)) %>% 
  add_row(ageM = 18, Condition = "Match", Prop = 2.0, n = 10) -> df_count
...
ggplot(df_count, aes(x = ageM)) +
  ...
  labs(x = "Age in months", y = "Proportion of correct responses (looking)") +
...
ggsave(file = here("Study1", "Figures", "Analysis1_PreferentialLokking_Binary_Age.png"), plot = gp, dpi = 350, width = 5.5, height = 4.8) # Figure 2a(left)
