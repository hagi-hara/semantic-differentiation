...
output_dir <- here("Study1", "MCMCSamples", "Analysis1_PreferentialLooking_Continuous_VocabularySize", "model1")
...
str_c(output_dir,
  ...
  read_stan_csv() -> fit1
print(fit1, pars = c("beta0", "beta1", "beta2", "beta3", "sigma", "f_kappa"), probs = c(0.5, 0.025, 0.975), digits=3)
...
output_dir <- here("Study1", "MCMCSamples", "Analysis1_PreferentialLooking_Continuous_VocabularySize", "model2")
if (!dir.exists(output_dir)){
  dir.create(output_dir, recursive = TRUE)
}
...
str_c(output_dir,
      list.files(output_dir, pattern = ".csv"), sep = "/") %>% 
  read_stan_csv() -> fit2
print(fit2, pars = c("beta0", "beta1", "beta2", "sigma", "f_kappa"), probs = c(0.5, 0.025, 0.975))
...
output_dir <- here("Study1", "MCMCSamples", "Analysis1_PreferentialLooking_Continuous_VocabularySize", "model3")
if (!dir.exists(output_dir)){
  dir.create(output_dir, recursive = TRUE)
}
...
str_c(output_dir,
      list.files(output_dir, pattern = ".csv"), sep = "/") %>% 
  read_stan_csv() -> fit3
print(fit3, pars = c("beta0", "beta1", "sigma", "f_kappa"), probs = c(0.5, 0.025, 0.975), digits=3)
...
output_dir <- here("Study1", "MCMCSamples", "Analysis1_PreferentialLooking_Continuous_VocabularySize", "model4")
if (!dir.exists(output_dir)){
  dir.create(output_dir, recursive = TRUE)
}
...
output_dir <- here("Study1", "MCMCSamples", "Analysis1_PreferentialLooking_Continuous_VocabularySize", "model5")
if (!dir.exists(output_dir)){
  dir.create(output_dir, recursive = TRUE)
}
...
df_waic %>% 
  ...
  arrange(desc(Estimate)) # see Table S5
...
data <- list(Nrow   = nrow(df),
             ...
             X1 = df$vcbAll,
             X2 = if_else(df$Condition == "Mis", 1, 0),
             # X3 = df$ageM * if_else(df$Condition == "Mis", 1, 0),
             N_subject  = length(unique(df$id)),
             ID_subject = df$ID_num,
             N_pred = nrow(df_new),
...
## MCMC sampling with cmdstanr
here("Study1", "Models", "Continuous", "model2_selected.stan") %>% 
  cmdstan_model() -> model
...
output_dir <- here("Study1", "MCMCSamples", "Analysis1_PreferentialLooking_Continuous_VocabularySize", "model_selected")
if (!dir.exists(output_dir)){
  dir.create(output_dir, recursive = TRUE)
}
...
print(gp)
ggsave(file = here("Study1", "Figures", "Analysis1_PreferentialLooking_Continuous_VocabularySize.png"), plot = gp, dpi = 350, width = 5.1, height = 4.9) # Figure S3b(left)
