...
output_dir <- here("Study1", "MCMCSamples", "Analysis2_PreferentialLooking_Continuous_Age", "model1")
...
output_dir <- here("Study1", "MCMCSamples", "Analysis2_PreferentialLooking_Continuous_Age", "model2")
if (!dir.exists(output_dir)){
  dir.create(output_dir, recursive = TRUE)
}
...
output_dir <- here("Study1", "MCMCSamples", "Analysis2_PreferentialLooking_Continuous_Age", "model3")
if (!dir.exists(output_dir)){
  dir.create(output_dir, recursive = TRUE)
}
...
output_dir <- here("Study1", "MCMCSamples", "Analysis2_PreferentialLooking_Continuous_Age", "model4")
if (!dir.exists(output_dir)){
  dir.create(output_dir, recursive = TRUE)
}
...
output_dir <- here("Study1", "MCMCSamples", "Analysis2_PreferentialLooking_Continuous_Age", "model5")
if (!dir.exists(output_dir)){
  dir.create(output_dir, recursive = TRUE)
}
...
df_waic %>% 
  ...
  arrange(desc(Estimate)) # see Table S4
...
data <- list(Nrow   = nrow(df),
             ...
             N_pred = nrow(df_new),
             X1_pred = df_new$ageM,
             X2_pred = if_else(df_new$Condition == "ObjAbs", 1, 0))
...
output_dir <- here("Study1", "MCMCSamples", "Analysis2_PreferentialLooking_Continuous_Age", "model_selected")
if (!dir.exists(output_dir)){
  dir.create(output_dir, recursive = TRUE)
}
...
df %>% 
  ...
  geom_jitter(aes(y = PropLook_Correct, color = Condition), shape = 16, size = 2, width = 0.05, height = 0, alpha = 0.5) +
  ...
  labs(x = "Age in months", y = "Mean proportion of looking at the correct stimulus") +
...
ggsave(file = here("Study1", "Figures", "Analysis2_PreferentialLooking_Continuous_Age.png"), plot = gp, dpi = 350, width = 5.0, height = 4.8) # Figure S3a(right)
