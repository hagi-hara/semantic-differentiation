...
d %>% 
  ...
  summarise(ageM = first(ageM),
            ...
            .groups = "drop") %>%
  filter(Condition == "Match" | Condition == "Mis") %>% 
...
output_dir <- here("Study2", "MCMCSamples", "Analysis1_PreferentialLooking_Binary", "model1")
...
output_dir <- here("Study2", "MCMCSamples", "Analysis1_PreferentialLooking_Binary", "model2")
if (!dir.exists(output_dir)){
  dir.create(output_dir, recursive = TRUE)
}
...
output_dir <- here("Study2", "MCMCSamples", "Analysis1_PreferentialLooking_Binary", "model3")
if (!dir.exists(output_dir)){
  dir.create(output_dir, recursive = TRUE)
}
...
output_dir <- here("Study2", "MCMCSamples", "Analysis1_PreferentialLooking_Binary", "model4")
if (!dir.exists(output_dir)){
  dir.create(output_dir, recursive = TRUE)
}
...
output_dir <- here("Study2", "MCMCSamples", "Analysis1_PreferentialLooking_Binary", "model5")
if (!dir.exists(output_dir)){
  dir.create(output_dir, recursive = TRUE)
}
...
## MCMC sampling with cmdstanr
here("Study2", "Models", "Binary", "model2_selected.stan") %>% 
  cmdstan_model() -> model
...
output_dir <- here("Study2", "MCMCSamples", "Analysis1_PreferentialLooking_Binary", "model_selected")
if (!dir.exists(output_dir)){
  dir.create(output_dir, recursive = TRUE)
}
...
ggs(fit) %>% 
  ...
  select(Condition, time, MED, lwr, upr) %>% 
  arrange(Condition, time) -> df_predict
df_predict
...
df %>%
  mutate(Prop = Trial_Correct/Trial_Total,
         ...
         Condition = if_else(Condition == "Mis", "Mismatch", Condition)) -> df_obs
...
df_obs %>% 
  ...
  ungroup() %>% 
  filter(!is.na(Prop)) %>% 
  add_row(time = "1st", Condition = "Match", Prop = 2.0, n = 13) -> df_count
...
ggsave(file = here("Study2", "Figures", "Analysis1_PreferentialLokking_Binary.png"), plot = gp, dpi = 350, width = 5.5, height = 4.8) # Figure 3a
