...
model{
  diff_words ~ poisson_log(beta0 + beta1 * X1 + beta2 * X2 + beta3 * X3 + beta4 * X4 + r);
  r ~ normal(0, sigma);
...
generated quantities{
  ...
  for(k in 1:Nrow){
    log_lik[k] = poisson_log_lpmf(diff_words[k] | beta0 + beta1 * X1[k] + beta2 * X2[k] + beta3 * X3[k] + beta4 * X4[k] + r[k]);
  }
}
